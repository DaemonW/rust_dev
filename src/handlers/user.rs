...
use crate::models::response::ServiceResult;
...
pub async fn list_user(pool: web::Data<DbPool>, params: web::Query<UserFilter>) -> ServiceResult {
    ...
    match query.load::<User>(conn) {
        Ok(ret) => ServiceResponse::ok(ret),
        Err(e) => {
            error!("{}", e);
            ServiceResponse::intern_err(ServiceError::ErrDbQueryFailed, "query users info failed")
        }
    }
}
...
pub async fn get_user(pool: web::Data<DbPool>, path: web::Path<String>) -> ServiceResult {
    ...
    match result {
        ...
        Err(e) => {
            error!("{}", e);
            ServiceResponse::intern_err(ServiceError::ErrDbQueryFailed, "query user info failed")
        }
    }
}
...
pub async fn delete_user(pool: web::Data<DbPool>, _id: web::Path<String>) -> ServiceResult {
    ...
    if user_id < 0 {
        ServiceResponse::bad_request(ServiceError::ErrIllegalPathParam, "illegal user id")
    } else {
        let conn = &mut pool.get().unwrap();
        let result =
            diesel::delete(schema::users::dsl::users.filter(schema::users::id.eq(user_id)))
                .execute(conn);
        match result {
            Ok(_) => ServiceResponse::ok("delete user success"),
            Err(e) => {
                error!("{}", e);
                ServiceResponse::intern_err(ServiceError::ErrDbDeleteFailed, "delete user failed")
            }
        }
    }
}
...
pub async fn update_user_info(
    ...
    if id < 0 {
        return ServiceResponse::bad_request(ServiceError::ErrIllegalPathParam, "illegal user id");
    }
    ...
    if let Err(e) = check_update_params(&params) {
        return ServiceResponse::bad_request(ServiceError::ErrIllegalQueryParam, e);
    }
    let conn = &mut _pool.get().unwrap();
    ...
    return match result {
        Ok(n) => {
            if n > 0 {
                ServiceResponse::ok("update user success")
            } else {
        ...
        Err(e) => {
            error!("{}", e);
            ServiceResponse::intern_err(ServiceError::ErrDbUpdateFailed, "update user failed")
        }
    };
}
...
pub async fn create_user(
    ...
    if let Err(e) = check_user_param(&register) {
        return ServiceResponse::bad_request(ServiceError::ErrIllegalBodyData, e);
    }
    ...
    let resp = match result {
        ...
        Err(e) => {
            if let SqlErr::DatabaseError(SqlErrKind::UniqueViolation, _) = e {
                ServiceResponse::bad_request(ServiceError::ErrDbRecordExist, "user already exists")
            } else {
                error!("{}", e);
                ServiceResponse::intern_err(ServiceError::ErrDbInsertFailed, "register user failed")
            }
        }
    };
    return resp;
...
fn check_user_param<'a>(info: &'a RegisterUserInfo) -> Result<(), &'static str> {
    ...
    pattern::username(info.username.as_ref().unwrap())?;
    pattern::password(info.password.as_ref().unwrap())?;
